...
class CourseRoleAudienceFilter(NotificationAudienceFilterBase):
    ...
    def filter(self, course_roles):
        ...
        if 'staff' in course_roles:
            staff_users = CourseStaffRole(self.course_key).users_with_role().values_list('id', flat=True)
            user_ids.extend(staff_users)

        if 'instructor' in course_roles:
            instructor_users = CourseInstructorRole(self.course_key).users_with_role().values_list('id', flat=True)
